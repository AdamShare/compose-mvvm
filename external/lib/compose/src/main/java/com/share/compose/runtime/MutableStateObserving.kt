 ...
 *
 * This delegate should be used within a class that implements [StateChangeObserver].
 * The backing state uses the specified [SnapshotMutationPolicy] to determine whether a change should be logged.
 *
...
): ReadWriteProperty<StateChangeObserver, T> = MutableStateObserving(
 ...
 */
fun mutableIntStateObservingOf(
    value: Int,
): ReadWriteProperty<StateChangeObserver, Int> = MutableStateObserving(
    equivalent = { a, b -> a == b },
    stateFactory = { mutableIntStateOf(value) }
)

/**
 * Creates a [MutableState] delegate for [Long] that reports changes through [StateChangeObserverRegistry].
 */
fun mutableLongStateObservingOf(
    value: Long,
): ReadWriteProperty<StateChangeObserver, Long> = MutableStateObserving(
    equivalent = { a, b -> a == b },
    stateFactory = { mutableLongStateOf(value) }
)

/**
 * Creates a [MutableState] delegate for [Double] that reports changes through [StateChangeObserverRegistry].
 */
fun mutableDoubleStateObservingOf(
    value: Double,
): ReadWriteProperty<StateChangeObserver, Double> = MutableStateObserving(
    equivalent = { a, b -> a == b },
    stateFactory = { mutableDoubleStateOf(value) }
)

/**
 * Creates a [MutableState] delegate for [Float] that reports changes through [StateChangeObserverRegistry].
 */
fun mutableFloatStateObservingOf(
    value: Float,
): ReadWriteProperty<StateChangeObserver, Float> = MutableStateObserving(
    equivalent = { a, b -> a == b },
    stateFactory = { mutableFloatStateOf(value) }
)
 ...
 */
class MutableStateObserving<T>(
    private val equivalent: (T, T) -> Boolean,
    stateFactory: () -> MutableState<T>,
     ...
     *
     * If the owning instance implements [StateChangeObserver], it will be registered and tracked.
     * If not, logging is disabled and a warning is emitted.
     */
