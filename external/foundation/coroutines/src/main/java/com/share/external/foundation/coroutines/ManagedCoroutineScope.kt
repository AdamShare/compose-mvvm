...
/**
 * A [CoroutineScope] manager that supports hierarchical child scopes and optionally defers
 * its own cancellation until all active child scopes have completed.
 *
 * This interface acts as both a contract for creating custom child scopes (like a
 * [CoroutineScopeFactory]) and a container for those child scopes. It integrates cancellation
 * logic so that the parent scope can decide whether to cancel immediately or wait until all
 * children finish.
 *
 * ### Basic Concept
 ...
 *
 * ### Thread Safety
 * - Implementations must ensure modifications to the set of active children are synchronized,
 *   so that registration and unregistration remain thread-safe.
 *
 * ### Usage
 * While this class can be applied to many scenarios that require hierarchical coroutine
 * scopes, one illustrative example is a **navigation stack**:
 *
 ...
 *    - If the parent is still active, the child starts; otherwise, the child is
 *      immediately cancelled.
 * 3. **Pop the screen** (or otherwise decide to remove it):
 *    - Call [cancel] on the parent with `awaitChildrenComplete = true` if you need
 *      to wait for all child screens to complete before final cancellation.
 *    - Or call [cancel] without waiting if you want to terminate immediately.
 *
 * ### `isActive` Usage
 * - If [isActive] is `false`, calls to [cancel] do nothing, and any new [childManagedScope]
 *   creation cancels the child immediately.
 * - If [isActive] is `true`, the parent is still functional, and children can be registered or
 *   cancelled normally.
 *
 * ### Cancellation Logic
 * - **Immediate Cancel**: Calling [cancel] with `awaitChildrenComplete = false` clears all
 *   children, cancels them, and then cancels the scope’s job—unless the parent is already inactive.
 * - **Deferred Cancel**: If `awaitChildrenComplete = true` and the scope has active children,
 *   the parent defers its cancellation until all those children unregister themselves
 *   (by finishing or being cancelled).
 * - **Multiple Calls**:
 *   1. If the parent is already waiting, further calls to [cancel] with `awaitChildrenComplete = false`
 *      will override that waiting, **unless** the scope is already inactive.
 *   2. If the scope is already inactive, any new call to [cancel] is a no-op.
 *
 * ### Cancellation vs. Completion
 * - **Cancelled**: The parent’s job has been explicitly requested to stop (via [cancel]).
 *   Eventually, the job becomes completed.
 * - **Completed**: The job has fully finished for any reason (including normal completion
 *   or final cancellation). Once the scope is completed, [isActive] is always `false`.
 */
interface ManagedCoroutineScope : CoroutineScopeFactory {

    /**
     * `true` if this scope can still launch coroutines and manage child scopes.
     * Once false, the scope is no longer functional.
     */
    val isActive: Boolean

    /**
     * Registers a completion handler on this scope’s job, which is triggered when the job
     * finishes or is cancelled. This is the same mechanism available in regular [Job]
     * instances via `Job.invokeOnCompletion`.
     *
     ...
     *
     * If the parent’s job is already completed or cancelled, the new child is immediately
     * cancelled. Otherwise, it is registered as an active child, and the parent waits
     * for it to complete before unregistration.
     *
     ...
     *
     * - If [awaitChildrenComplete] is `false`, or if there are no active children, this function
     *   clears all children and cancels the parent scope immediately.
     * - If [awaitChildrenComplete] is `true` and the scope has active children, the parent
     *   defers its cancellation until all children complete. If a non-blank [message] is
     *   provided on the first call, that message is recorded for final cancellation.
     * - Additional or repeated calls while the parent is waiting can override waiting by
     *   passing `awaitChildrenComplete = false`, unless the scope is already inactive.
     *
     * @param awaitChildrenComplete If `true`, postpone cancellation until current children finish.
     * @param message An optional reason for cancellation; the first non-blank message is retained
...
fun ManagedCoroutineScope(
    actual: CoroutineScope
): ManagedCoroutineScope = ManagedCoroutineScopeImpl(actual)

/**
 * Private implementation of [ManagedCoroutineScope]. Exposes no public API outside the interface.
 */
private class ManagedCoroutineScopeImpl(
    private val actual: CoroutineScope
) : ManagedCoroutineScope {
