...
/**
 * Creates a child [CoroutineScope] from the calling [CoroutineScope], using a standard [Job]
 * as the parent job. This means any failure in the child scope will cancel all children
 * of that scope, propagating up as part of structured concurrency.
 *
 * This is useful if you want strict failure handling, where one failing child cancels
 * its sibling coroutines in the same scope.
 *
 ...
 */
fun CoroutineScope.childJobScope(
    name: String = "",
    context: CoroutineContext = EmptyCoroutineContext
) = childScope(
    name = name,
    context = context,
    childJob = { Job(parent = it) }
)

/**
 * Creates a child [CoroutineScope] from the calling [CoroutineScope], using a [SupervisorJob]
 * as the parent job. With a supervisor job, a failure in one child does not automatically
 * cancel other children of the same supervisor scope.
 *
 * This is useful for scenarios where you want independent error handling among sibling
 * coroutines, allowing them to continue running even if one fails.
 *
 * @param name A human-readable label appended to the parent's [CoroutineName], if present.
 * @param context Additional [CoroutineContext] elements, defaulting to [EmptyCoroutineContext].
 * @return A new [CoroutineScope] that inherits its parent job and context, plus a new child [SupervisorJob].
 */
fun CoroutineScope.childSupervisorJobScope(
    name: String = "",
    context: CoroutineContext = EmptyCoroutineContext
) = childScope(
    name = name,
    context = context,
    childJob = { SupervisorJob(parent = it) }
)

/**
 * Internal helper function for creating a child [CoroutineScope]. It merges a new [Job] (provided by
 * [childJob]) with the caller's existing context ([this.coroutineContext]), plus an optional
 * [CoroutineContext].
 *
 * It also sets up a [CoroutineName] that appends [name] to the parent's coroutine name (if any),
